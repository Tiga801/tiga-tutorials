...
> **环境**：openEuler / aarch64 (ARM 架构)
> **最后更新**：2026-03-24

## 概述

针对 openEuler (aarch64/ARM 架构) 的完整服务器初始配置教程，涵盖用户管理、Docker 权限、SSH 免密登录、Zsh 定制（含 fzf 修复）及 Mihomo 代理配置。openEuler 使用 `wheel` 组管理 sudo 权限，包管理器为 `dnf`，软件源未收录 `fzf`，需通过 Git 源码安装。
...
openEuler 软件源未收录 `fzf`，通过官方 Git 方式安装：
